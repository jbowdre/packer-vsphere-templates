/*
  Red Hat Enterprise Linux 9 template using the Packer Builder for VMware vSphere (vsphere-iso).
*/
...
locals {
  ...
  rhsm_pool                     = vault("packer/data/rhsm",             "pool")                 // Pool ID containing RHSM subscriptions
  rhsm_username                 = vault("packer/data/rhsm",             "username")             // Username for authenticating to RHSM
...
local "rhsm_password" {
  expression                    = vault("packer/data/rhsm",             "password")             // Password for authenticating to RHSM
  sensitive                     = true
}
...
locals {
  ...
  build_description             = "Red Hat Enterprise Linux 9 template\nBuild date: ${local.build_date}\nBuild tool: ${local.build_tool}"
