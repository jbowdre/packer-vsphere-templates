#!/bin/bash -eu
# Performs steps to harden Ubuntu 20.04 LTS toward the CIS Level 2 benchmark
...
rule_name="Set Deny For Failed Password Attempts"
current_task "$rule_name"
password_pam_tally_retries='3'
password_pam_tally_time='600'
cat << EOF | sudo tee /usr/share/pam-configs/tally2
Name: tally2 lockout
Default: yes
Priority: 1024
Conflicts: cracklib
Auth-Type: Primary
Auth-Initial:
    required                        pam_tally2.so onerr=fail silent audit deny=$password_pam_tally_retries unlock_time=$password_pam_tally_time
...
if ! grep -q '^database=file:' ${AIDE_CONFIG}; then
  echo "database=file:${AIDE_DB}" | sudo tee -a ${AIDE_CONFIG}
else
  sudo sed -i "s|^database=file:.*$|database=file:${AIDE_DB}|" ${AIDE_CONFIG}
fi
